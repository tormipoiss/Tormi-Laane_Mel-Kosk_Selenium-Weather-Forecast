...
{
    internal class Multiple_days_over_limit
    {
        IWebDriver driver;
        [SetUp]
        ...
        public void Test()
        {
            ...
            driver.FindElement(By.Id("DayAmount")).SendKeys("67");
            ...
            string err = driver.FindElement(By.XPath("//p[contains(text(),'Cannot forecast weather longer than')]")).Text;
            //string realPlace = driver.FindElement(By.Id("weatherAddress")).Text;
            Assert.That(driver.Url == "https://localhost:5001/Home/City");
            Assert.That(tbCount == 15);
            Assert.That(err.Contains("Cannot forecast weather longer than"));
