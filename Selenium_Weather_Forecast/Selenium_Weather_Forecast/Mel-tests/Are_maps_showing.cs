...
{
    internal class Are_maps_showing
    {
        IWebDriver driver;
        [SetUp]
        ...
        public void Test()
        {
            ...
            string[] buttons = new string[] { "weatherGraphSpan", "googlemapSpan", "lightningmapSpan", "weathermapSpan" };
            string[] showNames = new string[] { "Show weather graph", "Show google map", "Show lightning map", "Show weather map" };
            string[] closeNames = new string[] { "Hide weather graph", "Hide google map", "Hide lightning map", "Hide weather map" };
            for (int i = 0; i < buttons.Length; i++)
            {
                var btn = driver.FindElement(By.Id(buttons[i]));
                Assert.That(btn.Text == showNames[i]);
                btn.Click();
                Assert.That(btn.Text == closeNames[i]);
                btn.Click();
                Assert.That(btn.Text == showNames[i]);
            }
